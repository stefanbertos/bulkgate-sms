...
describe('sendSMS', () => {
    ...
    it('should apply defaults for optional fields', async () => {
        (axios.post as jest.Mock).mockResolvedValueOnce({ data: { data: { status: 'accepted', sms_id: 'id', part_id: ['id_1'], number: '420123456789' } } });
        const minimal = {
    ...
    it('should handle error response', async () => {
        (axios.post as jest.Mock).mockResolvedValueOnce({ data: { type: 'invalid_phone_number', code: 400, error: 'Invalid phone number', detail: null } });
        const minimal = {
            application_id: 'app_id',
            application_token: 'token',
            number: 'invalid',
